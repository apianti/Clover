...
IntelCpuGetNumberOfProcessors (
  ...
  UINTN  LogicalProcessors = 1;
  UINTN  PhysicalProcessors = 1;
  UINT32 MaxFunction = 0;
  UINT32 Eax = 0;
  UINT32 Ebx = 0;
  UINT32 SubFunction = 0;
  // Check parameters
  ...
  EfiCpuid(0, &MaxFunction, NULL, NULL, NULL);
  if (MaxFunction >= 0xB) {
    // Perform topology query
    EfiCpuidEx(0xB, SubFunction, &Eax, &Ebx, NULL, NULL);
    while (EFI_BITFIELD(Ebx, 0, 15) != 0) {
      // EBX[0:15] is the number of logical processors at this physical level
      LogicalProcessors *= EFI_BITFIELD(Ebx, 0, 15);
      // EAX[0:4] is the number of bits to shift to next physical level
      PhysicalProcessors <<= EFI_BITFIELD(Eax, 0, 4);
      EfiCpuidEx(0xB, ++SubFunction, &Eax, &Ebx, NULL, NULL);
    };
  } else if (MaxFunction >= 0x4) {
    // Perform cache query
    EfiCpuid(0x4, &Eax, NULL, NULL, NULL);
    PhysicalProcessors = EFI_BITFIELD(Eax, 26, 31);
    EfiCpuid(0x1, NULL, &Ebx, NULL, NULL);
    LogicalProcessors = PhysicalProcessors * EFI_BITFIELD(Ebx, 16, 23);
  } else if (MaxFunction >= 0x1) {
    // Assume the count information in EBX[16:23] is correct
    EfiCpuid(1, NULL, &Ebx, NULL, NULL);
    LogicalProcessors = (UINTN)EFI_BITFIELD(Ebx, 16, 23);
    PhysicalProcessors = LogicalProcessors;
  }
  // Return the number of processors on this package
  *NumberOfLogicalProcessors = LogicalProcessors;
  *NumberOfPhysicalProcessors = PhysicalProcessors;
  // Assume there is only one package
  *NumberOfPackages = 1;
  return EFI_SUCCESS;
}
