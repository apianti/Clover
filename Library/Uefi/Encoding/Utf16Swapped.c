///
/// @file Library/Uefi/Encoding/Utf16Swapped.c
///
/// UTF-16 byte swapped character encoding
///
...
STATIC EFI_HANDLE mUtf16SwappedHandle = NULL;

// gEfiUtf16SwappedEncodingProtocolGuid
/// The UTF-16 byte swapped character string encoding protocol unique identifier
EFI_GUID gEfiUtf16SwappedEncodingProtocolGuid = EFI_UTF16_SWAPPED_ENCODING_PROTOCOL_GUID;
...
EfiUtf16SwappedEncoding (
  ...
  if (EFI_ERROR(EfiLocateProtocol(&gEfiUtf16SwappedEncodingProtocolGuid, NULL, (VOID **)&Encoding))) {
    return NULL;
  }
  return Encoding;
}

// Utf16SwappedDecodeCharacter
...
Utf16SwappedDecodeCharacter (
  ...
  do {
    ...
    // Swap byte order of character unit
    Unit = EfiByteSwap16(Unit);
...
Utf16SwappedEncodeCharacter (
  ...
  if (Character > 0xFFFF) {
    ...
    if ((Utf16 != NULL) && (*Size >= (2 * sizeof(CHAR16)))) {
      ...
      Character -= 0x10000;
      // Encode the high/leading surrogate while swapping the byte order
      Unit = (CHAR16)(0xD800 + EFI_BITFIELD(Character, 0, 9));
      *Utf16++ = EfiByteSwap16((UINT16)Unit);
      // Encode the low/trailing surrogate while swapping the byte order
      Unit = (CHAR16)(0xDC00 + EFI_BITFIELD(Character, 0, 9));
      *Utf16 = EfiByteSwap16((UINT16)Unit);
    } else {
    ...
    *Size = (2 * sizeof(CHAR16));
    ...
    if ((Utf16 != NULL) && (*Size >= sizeof(CHAR16))) {
      // Swap byte order of character unit
      *Utf16 = EfiByteSwap16((UINT16)Character);
    } else {
      Status = EFI_BUFFER_TOO_SMALL;
    }
...
Utf16SwappedEncodingUninstall (
  ...
  if ((mUtf16SwappedHandle == NULL) || EFI_ERROR(EfiLocateProtocol(&gEfiUtf16SwappedEncodingProtocolGuid, NULL, (VOID **)&Encoding)) || (Encoding == NULL)) {
    return EFI_SUCCESS;
  }
  // Uninstall and free the protocol on error
  Status = EfiUninstallMultipleProtocolInterfaces(mUtf16SwappedHandle, &gEfiUtf16SwappedEncodingProtocolGuid, Encoding, &gEfiEncodingProtocolGuid, Encoding, NULL);
...
Utf16SwappedEncodingInstall (
  ...
  if (!EFI_ERROR(EfiLocateProtocol(&gEfiUtf16SwappedEncodingProtocolGuid, NULL, (VOID **)&Encoding)) && (Encoding != NULL)) {
    return EFI_SUCCESS;
  }
  ...
  Encoding->Name = "UTF-16-BS";
  Encoding->UnitSize = sizeof(CHAR16);
  Encoding->MaxUnits = 2;
  Encoding->DecodeCharacter = Utf16SwappedDecodeCharacter;
  Encoding->EncodeCharacter = Utf16SwappedEncodeCharacter;
  // Install the encoding protocol
  Status = EfiInstallMultipleProtocolInterfaces(&mUtf16SwappedHandle, &gEfiUtf16SwappedEncodingProtocolGuid, Encoding, &gEfiEncodingProtocolGuid, Encoding, NULL);
  if (EFI_ERROR(Status)) {
    // Uninstall and free the protocol on error
    EfiUninstallMultipleProtocolInterfaces(mUtf16SwappedHandle, &gEfiUtf16SwappedEncodingProtocolGuid, Encoding, &gEfiEncodingProtocolGuid, Encoding, NULL);
    EfiFreePool((VOID *)Encoding);
  }
  return Status;
}
